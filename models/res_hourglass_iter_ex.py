...
class Network(nn.Module):
    def __init__(self, config):
      ...
      self.extractor    = nn.ModuleList( [nn.Conv2d(2 * config['f'], config['f'], 1) for i in range(self.max_stack-1)])
      ...
      self.beta  = nn.Parameter(torch.zeros(self.max_stack - 1))
      self.ReLU  = nn.LeakyReLU(inplace=True)
      
      # Normalization layers
      if   config['normalization'] == 'batch':
        self.bn1 = nn.ModuleList([nn.BatchNorm2d(config['f']) for i in range(self.max_stack)])
        self.bn2 = nn.BatchNorm2d(config['num_class'])
        self.bn3 = nn.BatchNorm2d(config['f'])
      elif config['normalization'] == 'layerHW':
        self.bn1 = nn.ModuleList([nn.LayerNorm([config['oup_dim'][0], config['oup_dim'][1]]) for i in range(self.max_stack)])
        ...
        self.bn1 = nn.ModuleList([nn.LayerNorm([config['f'], config['oup_dim'][0], config['oup_dim'][1]]) for i in range(self.max_stack)])
    ...
    def forward(self, x):
      ...
      for i in range(self.max_stack):
        ...
        if i == 0:
          ...
          hidden_con = self.ReLU(self.bn1[i-1](self.extractor[i-1](hidden_cat)))
          
          hidden = self.hgs[i](self.beta[i-1] * residual + self.alpha[i-1] * hidden_con + prev_hidden)
