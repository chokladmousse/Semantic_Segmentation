...
class Network(nn.Module):
    def __init__(self, config):
      ...
      middle_dim1       = [int(x/2) for x in config['inp_dim']]
      middle_dim2       = [int(x/2) for x in middle_dim1]
      config['oup_dim'] = [int(x/2) for x in middle_dim2]
      config['scale']   = 8
      ...
      self.conv5 = nn.Conv2d(config['f'], config['f'], 5, padding=2)
      self.conv6 = nn.Conv2d(config['f'], config['f'], 5, padding=2)
      ...
      if   config['normalization'] == 'batch':
        ...
        self.bn5 = nn.BatchNorm2d(config['f'])
        self.bn6 = nn.BatchNorm2d(config['f'])
        ...
        self.bn3 = nn.LayerNorm([middle_dim1[0], middle_dim1[1]])
        self.bn4 = nn.LayerNorm([middle_dim1[0], middle_dim1[1]])
        self.bn5 = nn.LayerNorm([middle_dim2[0], middle_dim2[1]])
        self.bn6 = nn.LayerNorm([middle_dim2[0], middle_dim2[1]])
        ...
        self.bn3 = nn.LayerNorm([config['f'], middle_dim1[0], middle_dim1[1]])
        self.bn4 = nn.LayerNorm([config['f'], middle_dim1[0], middle_dim1[1]])
        self.bn5 = nn.LayerNorm([config['f'], middle_dim2[0], middle_dim2[1]])
        self.bn6 = nn.LayerNorm([config['f'], middle_dim2[0], middle_dim2[1]])
      else:
    ...
    def forward(self, x0):
      ...
      z0 = self.pool2(y2)
      z1 = self.ReLU(self.bn5(self.conv5(z0)))
      z2 = self.ReLU(self.bn6(self.conv6(z1)))
      out = self.pool2(z2)
      return out
